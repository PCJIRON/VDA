...
Combines self-calibration, multi-scale VDA ensemble, visual centroid,
...
class PerfectClicker:
    ...
    def __init__(self, api_url: str, api_key: str, model: str = "vda-vl-plus",
    ...
    def _ask_vda_single(self, img_bgr: np.ndarray, target: str, send_w: int, send_h: int) -> List[Dict[str, Any]]:
        """Single-scale VDA query."""
...
"""
    ...
    def _ask_vda_ensemble(self, img_bgr: np.ndarray, target: str) -> List[Dict[str, Any]]:
        """Multi-scale VDA query with ensemble merging."""
        h, w = img_bgr.shape[:2]
        results_all = self._ask_vda_single(img_bgr, target, w, h)

        zoom_factor = 1.5
        zoomed = cv2.resize(img_bgr, (int(w * zoom_factor), int(h * zoom_factor)), interpolation=cv2.INTER_LANCZOS4)
        zoom_results = self._ask_vda_single(zoomed, target, int(w * zoom_factor), int(h * zoom_factor))
    ...
    def click(self, target: str, region: Optional[Tuple[int, int, int, int]] = None,
        ...
        for attempt in range(1, retries + 1):
            ...
            try:
                ...
                results = self._ask_vda_ensemble(img_bgr, target)
                if not results:
                    logger.warning("[MISS] VDA: not found")
                    time.sleep(1)
                    continue
