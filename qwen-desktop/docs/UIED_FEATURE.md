...
UIED (UI Element Detection) is a powerful feature that captures screenshots, detects UI components using OpenCV, and labels them intelligently using Qwen Vision LLM. It provides **100% accurate coordinate matching** through template matching.
...
- **LLM Labeling**: Qwen Vision labels each component intelligently
...
5. Annotated image created (numbered boxes)
   │
   ▼
6. Sent to Qwen Vision for labeling
   │
   ▼
...
service = UIEDService(
    ...
    template_dir="~/.qwen_desktop/uied_templates"
)
...
- Qwen Vision understands context
